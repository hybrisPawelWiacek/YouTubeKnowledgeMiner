...
The application implements a comprehensive, production-ready logging system that provides detailed visibility into all aspects of application behavior, with a particular focus on authentication flows and API interactions.

### Core Components

- **Centralized Logger:** Based on Winston, with environment-aware configuration for development and production
- **Multiple Specialized Loggers:** Dedicated loggers for API interactions, authentication events, and general application logs
- **Log Rotation:** Daily log files with automatic rotation and retention policies
- **Request Correlation:** Every HTTP request receives a unique ID for cross-component tracing
- **Sensitive Data Redaction:** Automatic filtering of sensitive information like passwords and tokens
- **Environment Detection:** Adapts logging behavior based on detected environment (development vs. production)
- **Console Redirection:** Intercepts standard console.* methods to ensure consistent logging format

### Log Types and Destinations

- **Combined Logs:** General application activity (`combined-YYYY-MM-DD.log`)
- **Error Logs:** Error-level events only (`error-YYYY-MM-DD.log`)
- **API Logs:** Detailed API request/response tracking (`api-YYYY-MM-DD.log`)
- **Authentication Logs:** Authentication and security events (`auth-YYYY-MM-DD.log`)
- **Debug Logs:** Special non-rotated debug logs for troubleshooting (`auth-debug.log`)
- **Console Output:** All logs also appear in the console with appropriate coloring in development

### Middleware Stack

- **Request ID Middleware (`request-id.middleware.ts`):**
  - Generates a unique UUID for each request
  - Attaches ID to request object (`req.requestId`)
  - Adds ID to response headers (`X-Request-ID`)
  - Enables request tracing across different log files

- **Logging Middleware (`logging.middleware.ts`):**
  - `requestLogger`: Logs incoming HTTP requests with appropriate detail level
  - `responseLogger`: Logs HTTP responses including timing information
  - `errorLogger`: Captures and logs unhandled errors in the request pipeline
  - Intelligently filters static asset requests to reduce log noise

### Core Logging Functions

- **logApiRequest:** Detailed API request logging with sensitive data redaction
- **logApiResponse:** Detailed API response logging with timing information
- **logAuthEvent:** Authentication event tracking with standard format
- **logSecurityEvent:** Security-related events like access denials or suspicious activity
- **flushLogs:** Ensures all logs are properly written to disk before application exit

### Log Data Structure

- **Standardized JSON Format:** In production, logs use a structured JSON format
- **Human-Readable Format:** In development, logs use a colorized, formatted output
- **Common Fields:**
  - `timestamp`: ISO-format timestamp
  - `level`: Log level (error, warn, info, debug)
  - `message`: Primary log message
  - `requestId`: Unique request identifier (when applicable)
  - `service`: Service/component identifier
  - `environment`: Runtime environment
  
- **Context-Specific Fields:**
  - API logs include method, URL, headers, request/response body
  - Auth logs include user ID, event type, and relevant authentication details
  - Error logs include error name, message, and stack trace

### API Integration

- **Logging API Routes (`logging.routes.ts`):**
  - `/api/logs`: Retrieve logs with filtering by level, date, and search term
  - `/api/logs/files`: List available log files with metadata
  - `/api/logs/:requestId`: Retrieve all logs for a specific request ID
  - Authentication required (with developer bypass option)

### Error Handling

- **Consistent Error Logging:** Standard format for all errors with appropriate context
- **Non-blocking Design:** Logging failures don't impact application operation
- **Stack Trace Preservation:** Full stack traces captured for errors
- **Error Categorization:** Errors are categorized by component and type

### Specialized Features

- **Console Method Interception:** Redirects `console.log`, `console.error`, etc. through Winston
- **Caller Detection:** Automatically captures the calling function/file for better context
- **Production Optimization:** Different formats and detail levels based on environment
- **Exit Handling:** Ensures logs are flushed to disk on application shutdown
- **Selective Filtering:** Intelligent filtering of high-volume or less useful log entries

### Implementation Notes

- **Environment Detection:** Multiple checks to reliably detect running environment
- **File Organization:** Logs stored in dedicated `/logs` directory with consistent naming
- **High Performance:** Optimized for minimal performance impact in production
- **Developer Experience:** Rich, colorized output in development environment
- **Testing Tools:** Specialized test scripts for verifying logging functionality

## Conclusion

This application represents a sophisticated modern web architecture using TypeScript throughout the stack. It leverages AI capabilities for natural language processing and semantic search, with a hybrid database approach that uses Neon PostgreSQL with pgvector for primary storage and vector operations. The dual-mode authentication system provides flexibility, allowing the application to function with or without Supabase integration while maintaining a smooth user experience. The comprehensive logging infrastructure ensures proper visibility into all application operations, helping with debugging, security monitoring, and performance analysis.
